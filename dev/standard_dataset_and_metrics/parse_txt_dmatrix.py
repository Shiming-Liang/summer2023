...
def get_best_movements(OP_formulation):
    ...
    """
    # hyperparams
    num_best_next = min(OP_formulation.graph['n'], 40)
...
def MOABC(num_bee, maximum_trial_num, maximum_itr, OP_formulation, label):
    ...
    for itr in tqdm(range(maximum_itr)):
        ...
        plt.legend()
        plt.pause(0.05)
    plt.savefig('results/'+label+'_{}_{}_{}.png'.format(
...
for txt_path in txt_paths:
    ...
    best_bees = MOABC(num_bee, maximum_trial_num, maximum_itr,
                      OP_formulation, txt_path.stem)
    with open("results/"+txt_path.stem+"_{}_{}_{}.pkl".format(num_bee, maximum_trial_num, maximum_itr), "wb") as fp:
