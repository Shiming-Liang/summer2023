...
def MOABC(num_bee, maximum_trial_num, maximum_itr):
    ...
    for itr in range(maximum_itr):
        # ---- Employed Bees Phase
        for employed_bee_idx, employed_bee in enumerate(tqdm(bees)):
        ...
        plt.legend()
        plt.pause(0.05)
    plt.savefig('results/main_v5_{}_{}_{}.png'.format(
...
for num_bee in [10, 20]:
    for maximum_trial_num in [1, 2, 3, 4, 5, 10, 20]:
        best_bees = MOABC(num_bee, maximum_trial_num, 100)
        with open("results/main_v5_{}_{}_100.pkl".format(num_bee, maximum_trial_num), "wb") as fp:
            pickle.dump(best_bees, fp)

"""
comment:
    the trial number reset when solution is updated, the employed bees and the
    onlooker bees share the same bees
"""
