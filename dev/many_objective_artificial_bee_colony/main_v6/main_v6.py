...
imputer = SimpleImputer(missing_values=np.nan,
                        strategy='constant', fill_value=5)
data_impute = imputer.fit_transform(data_miss)
data_impute[:, :3] = 5-data_impute[:, :3]
...
def MOABC(num_bee, maximum_trial_num, maximum_itr):
    ...
    for itr in range(maximum_itr):
        ...
        plt.legend()
        plt.pause(0.05)
    plt.savefig('results/main_v6_{}_{}_{}.png'.format(
...
# %% hyper-params trial
for num_bee in [20]:
    for maximum_trial_num in [30]:
        best_bees = MOABC(num_bee, maximum_trial_num, 1000)
        with open("results/main_v6_{}_{}_1000.pkl".format(num_bee, maximum_trial_num), "wb") as fp:
            pickle.dump(best_bees, fp)

"""
comment:
    the dataset imputation fixed according to the explanation of the geologists
"""
