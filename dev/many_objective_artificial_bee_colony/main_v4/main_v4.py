...
# for i in range(100):
#     list_of_bees.append(create_bee(OP_formulation))
# sorted_list_of_bees = multi_objectives_sort(list_of_bees)
...
# for i in range(100):
#     list_of_bees.append(create_bee(OP_formulation))
# list_of_bees, _ = rank(list_of_bees)
# list_of_bees = crowd(list_of_bees)
# selection_probability = get_probability(list_of_bees)


# %% MOABC formulation
def MOABC(num_bee, maximum_trial_num, maximum_itr):
    """
    MOABC implementation

    Parameters
    ----------
    num_bee : int
        number of employed/onlooker bees.
    maximum_trial_num : int
        abandon criterion.
    maximum_itr : int
    ...
    for itr in range(maximum_itr):
        ...
        employed_bees = multi_objectives_sort(employed_bees)
        # get probabiities
        ...
        for onlooker_bee_idx, onlooker_bee in enumerate(tqdm(onlooker_bees)):
            ...
            selected_employed_bee = rng.choice(
                employed_bees, p=selection_probability)
        ...
        for bee_idx, bee in enumerate(employed_bees):
            if bee['trial_num'] > maximum_trial_num:
                employed_bees[bee_idx] = create_bee(OP_formulation)
        # ---- Memorize the best solution achieved so far
        # multi-objective sort
        list_of_bees = employed_bees+onlooker_bees
        list_of_bees = multi_objectives_sort(list_of_bees)
        # save the best population as employed bees
        # log and save best obj for record
        best_bee = list_of_bees[0]
        best_bees.append(best_bee)
        best_objectives.append(best_bee['objectives'])
        plt.clf()
        plt.title('best scores vs. generation')
        for obj_idx in range(len(best_objectives[0])):
            plt.plot([itr_objectives[obj_idx]
                     for itr_objectives in best_objectives], label='objective %s' % obj_idx)
        plt.xlabel('generation')
        plt.ylabel('best scores of the generation')
        plt.legend()
        plt.pause(0.05)
    plt.savefig('results/main_v4_{}_{}_{}.png'.format(
        num_bee, maximum_trial_num, maximum_itr))


# %% hyper-params trials
for num_bee in [10, 20]:
    for maximum_trial_num in [1, 2, 3, 4, 5, 10, 20]:
        MOABC(num_bee, maximum_trial_num, 100)

"""
comment:
    onlooker bees don't influence the trial number'
"""
