...
class MyAStar(AStar):
    ...
    def __init__(self, topo, topo_np, debug, latlon_resolution):
        # cache topology dataset for index / xy switching
        self.topo = topo
        self.topo_np = topo_np
        self.elevation_resolution = 10
        self.debug = debug
        self.latlon_resolution = latlon_resolution  # step size to find the neighbor
        ...
        for row in range(row_c-self.latlon_resolution, row_c+self.latlon_resolution+1, self.latlon_resolution):
            ...
            for col in range(col_c-self.latlon_resolution, col_c+self.latlon_resolution+1, self.latlon_resolution):
                ...
                for elevation in [n[2]-self.elevation_resolution, n[2], n[2]+self.elevation_resolution]:
                    # skip identity
                    if row != row_c or col != col_c or elevation != n[2]:
                        # check occupancy
    ...
    def is_goal_reached(self, current, goal):
        ...
        if self.debug == True:
            self.ax.plot(row_c, col_c, current[2], 'ro')
            self.ax.plot(row_g, col_g, goal[2], 'go')
            self.count += 1
            if self.count == 100:
                self.count = 0
                self.ax.clear()
        ...
        if abs(row_c-row_g) < self.latlon_resolution:
            if abs(col_c-col_g) < self.latlon_resolution:
                if abs(current[2]-goal[2]) < self.elevation_resolution:
                    return True


# # %% unit test
# start_lat, start_long = 37.4622, -118.14
# goal_lat, goal_long = 37.4621, -118.14
...
# my_astar = MyAStar(topo, topo_np, False, 2)
# path = my_astar.astar(start, goal)
...
def get_path(start, goal):
    ...
    my_astar = MyAStar(topo, topo_np, False, 1)
