...
def mutate(OP_formulation, parents, population, pareto_set_approximation, num_to_add):
    ...
    partial_get_child = partial(get_child, OP_formulation, num_to_add)
    # apply multi-processing pool
    with Pool() as pool:
        children = pool.map(partial_get_child, parents)
    # update population and pareto_set_approximation
...
# get_children(OP_formulation, num_to_add, parent)
def get_child(OP_formulation, num_to_add, parent):
