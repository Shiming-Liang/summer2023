...
def select(pareto_set_approximation, backup_pool, minimum_population):
    """
    ...
    """
    # check len(pareto_set_approximation)
    if len(pareto_set_approximation) >= minimum_population:
        # if larger than minimum_population
        # get the objectives
        ...
        # compute the probability using sum log exp trick
        probability = np.exp(fitness-logsumexp(fitness))
        # select the parents using choice
        parents = rng.choice(
            list(pareto_set_approximation.values()), 2, p=probability)
        # elif smaller than minimum_population
    else:
        # check if it is neccessary to drop solutions in backup pool
        num_to_keep = minimum_population-len(pareto_set_approximation)
        if num_to_keep < len(backup_pool):
            # get the objectives
            objectives_of_all_solutions = []
            for solution in backup_pool.values():
                objectives_of_all_solutions.append(solution['objectives'])
            objectives_of_all_solutions = np.array(objectives_of_all_solutions)
            # normalize the objectives
            scaler = MinMaxScaler()
            ...
            fitness = compute_fitness(objectives_of_all_solutions)
            # drop the worst solutions in the backup pool
            keep_indices = np.argpartition(
                fitness, -num_to_keep)[-num_to_keep:]
            backup_pool_key_list = list(backup_pool)
            backup_pool = {
                backup_pool_key_list[keep_idx]: backup_pool[backup_pool_key_list[keep_idx]] for keep_idx in keep_indices}
        # create the mating pool as the union of pareto_set_approximation and
        # backup pool
        mating_pool = {**pareto_set_approximation, **backup_pool}
        # get the objectives
        objectives_of_all_solutions = []
        for solution in mating_pool.values():
            objectives_of_all_solutions.append(solution['objectives'])
        objectives_of_all_solutions = np.array(objectives_of_all_solutions)
        # normalize the objectives
        scaler = MinMaxScaler()
        ...
        # select the parents using choice
        parents = rng.choice(
            list(mating_pool.values()), 2, p=probability)

    return parents, backup_pool
...
def pareto_update(old_pareto_set_approximation, backup_pool, solution):
    """
    ...
    if key in old_pareto_set_approximation:
        ...
        updated = False
        # break
        return old_pareto_set_approximation, backup_pool, updated
    ...
    for another_key, another_solution in old_pareto_set_approximation.items():
        ...
        if (objectives >= another_objectives).all() and (objectives > another_objectives).any():
            ...
            # add another_solution to the backup pool
            if another_key not in backup_pool:
                backup_pool[another_key] = new_pareto_set_approximation[another_key]
        ...
        if (objectives <= another_objectives).all() and (objectives < another_objectives).any():
            # if another_solution dominate solution, set False updated
            updated = False
            # add the solution to the backup pool
            if key not in backup_pool:
                backup_pool[key] = solution
            # break
            break
    ...
    return new_pareto_set_approximation, backup_pool, updated
...
def drop(OP_formulation, old_solution, dropped_proportion, weight):
    ...
    drop_num_vertices = int((total_num_vertices-2)*dropped_proportion)
...
def local_search(OP_formulation, solution, weight, maximum_trial_number, pareto_set_approximation, backup_pool, dropped_proportion):
    """
    ...
    while trial_num < maximum_trial_number:
        ...
        pareto_set_approximation, backup_pool, updated = pareto_update(
            pareto_set_approximation, backup_pool, solution)
    ...
    return pareto_set_approximation, backup_pool
...
# ---- IBEA4MOOP
def IBEA4MOOP(OP_formulation, maximum_iteration, maximum_trial_number, dropped_proportion, minimum_population):
    ...
    for k in range(K):
        ...
        pareto_set_approximation, backup_pool = local_search(
            OP_formulation, solution, weight, maximum_trial_number, pareto_set_approximation, backup_pool, dropped_proportion)

    # while terminal condition not met
    for itr in range(maximum_iteration):
        # select
        parents, backup_pool = select(pareto_set_approximation,
                                      backup_pool, minimum_population)
        # crossover
        child, weight = crossover(OP_formulation, parents)
        # local search with the weight of the child solution
        pareto_set_approximation, backup_pool = local_search(
            OP_formulation, child, weight, maximum_trial_number, pareto_set_approximation, backup_pool, dropped_proportion)
...
txt_paths = list(
    Path("../../../dataset/moop/2 objectives/coord").rglob("2_p21_t023.[tT][xX][tT]"))
...
for txt_path in txt_paths:
    ...
    for run in range(runs_num):
        ...
        pareto_front_approximation, pareto_set_approximation = IBEA4MOOP(
            OP_formulation, maximum_iteration, maximum_trial_number, dropped_proportion, minimum_population)
        # write files
