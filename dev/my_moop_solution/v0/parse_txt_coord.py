...
def logsumexp(x):
    """
    Standard lop sum exp trick to avoid over/underflow.
...
def select(pareto_set_approximation):
    ...
    """
    # write the objectives as a np array
    objectives_of_all_solutions = []
    for solution in pareto_set_approximation:
    ...
    ref_point = np.zeros_like(unique[0, :])
...
def shorten(OP_formulation, old_solution):
    ...
    if OP_formulation.graph['start'] == OP_formulation.graph['end']:
        ...
        distance_matrix = np.pad(
            distance_matrix, ((1, 0), (1, 0)), 'constant', constant_values=np.inf)
    ...
    tsp_solution = elkai.DistanceMatrix(distance_matrix)
...
def drop(OP_formulation, old_solution, dropped_proportion, weight):
    ...
    # iterate through the permutation
    while next_vertex != OP_formulation.graph['end']:
...
def solution_from_visited_list(OP_formulation, visited_vertices):
    ...
    if visited_vertices[0] != visited_vertices[-1]:
        permutation[visited_vertices[-1]] = None
...
def local_search(OP_formulation, solution, weight, maximum_trial_number, pareto_set_approximation, dropped_proportion):
    ...
    while trial_num < maximum_trial_number:
        ...
        solution = drop(OP_formulation, solution, dropped_proportion)
        solution = add(OP_formulation, solution)
        ...
        if (updated == True) or (new_weighted_objectives > old_weighted_objectives):
            trial_num = 0
        else:
            trial_num += 1
...
def IBEA4MOOP(OP_formulation, maximum_iteration, maximum_trial_number, dropped_proportion):
    ...
    for itr in range(maximum_iteration):
        ...
        pareto_set_approximation = local_search(
            OP_formulation, child, weight, maximum_trial_number, pareto_set_approximation)
...
txt_paths = list(
    Path("../../../dataset/moop/2 objectives/coord").rglob("2_p66_t090.[tT][xX][tT]"))
...
for txt_path in txt_paths:
    ...
    for run in range(runs_num):
        ...
        pareto_set_approximation, pareto_set_approximation = IBEA4MOOP(
        ...
        # plot fronts
        if run < 5:
            pareto_set_approximation = pareto_set_approximation.sort_values(
                by=0)
            plt.step(
                pareto_set_approximation[0], pareto_set_approximation[1], 'o-')
