...
def crossover(OP_formulation, parents):
    ...
    # get the weight
    if np.any(child['objectives']):
        weight = child['objectives']
    else:
        weight = rng.random(child['objectives'].shape)
    weight = weight / np.sum(weight)
    return child, weight
...
def pareto_update(old_pareto_set_approximation, solution):
    ...
    for key, another_solution in old_pareto_set_approximation.items():
        ...
        if (objectives == another_objectives).all():
            if (solution['permutation'] == another_solution['permutation']).all():
                # if another_solution == solution, set False updated
                updated = False
                # break
                break
...
def add(OP_formulation, old_solution, weight):
    ...
    for curr_vertex, next_vertex in enumerate(permutation):
        ...
        if curr_vertex == next_vertex:
            ...
            while prev_vertex != OP_formulation.graph['end']:
                # create add_tuple
    ...
    # while true
    while pq_list:
        # create a list for negative_bcr
        negative_bcr_list = []
        # iterate through the list of priority queues
        for pq, _, _ in pq_list:
            # peek the item (add_tuple, negative_bcr)
            _, negative_bcr = pq.peek_smallest()
            # append to the list of peeked items
            negative_bcr_list.append(negative_bcr)
        # get the priority queue with the smallest negative_bcr
        idx_smallest_negative_bcr = np.argmin(negative_bcr_list)
        pq_with_smallest_negative_bcr = pq_list[idx_smallest_negative_bcr][0]
        # get the add tuple
        add_tuple = pq_with_smallest_negative_bcr.get_item()
        # del the priority queue with the smallest negative_bcr
        del pq_list[idx_smallest_negative_bcr]
        # copy the new solution into a temporary solution
        temporary_solution = deepcopy(new_solution)
        # add the vertex to the temporary solution
        temporary_solution = add_vertex(
            OP_formulation, temporary_solution, add_tuple)
        # if the temporary solution has route length larger than the constraint
        if temporary_solution['route_length'] > OP_formulation.graph['maximum_path_length']:
            # return the new solution
            return new_solution
        # overwrite the new solution with the temprary solution
...
for txt_path in txt_paths:
    ...
    # clear file
    open('results/'+txt_path.stem+'_MOABC_front', 'w').close()
    open('results/'+txt_path.stem+'_MOABC_set', 'w').close()
