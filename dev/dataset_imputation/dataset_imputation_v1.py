...
def get_impute_itr_forest_score(data_miss, data_full):
...
scores.append(get_impute_itr_forest_score(data_miss, data_full))
x_labels.append('iterative forest')


# iterative imputation
def get_impute_iterative(data_miss, data_full):
    tol = 1e-5
    imputer = IterativeImputer(
        random_state=0, max_iter=100, tol=tol
    )
...
scores.append(get_impute_iterative(data_miss, data_full))
...
for trial in range(num_trial):
    ...
    scores[3] += get_impute_itr_knn_score(data_miss, data_full)/num_trial
    scores[4] += get_impute_iterative(data_miss, data_full)/num_trial
    scores[5] += get_impute_mean(data_miss, data_full)/num_trial
...
plt.show()
plt.savefig('imputation_trials.jpg')
