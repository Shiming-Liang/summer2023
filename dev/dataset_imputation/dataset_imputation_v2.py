...
# KNeighborsRegressor+iterative imputation
...
# # knn imputation + itertaive imputation
# def get_impute_hybrid(data_miss, data_full):
#     tol = 1e-2

#     data_knn = data_miss.copy()
#     data_knn[:, -2:] *= 10000
#     data_itr = data_miss.copy()[:, :3]
#     imputer_knn = KNNImputer(missing_values=np.nan, n_neighbors=5)
#     imputer_itr = IterativeImputer(random_state=0, max_iter=100, tol=tol)

#     data_knn_impute = imputer_knn.fit_transform(data_knn)[:, :3]
#     data_itr_impute = imputer_itr.fit_transform(data_itr)

#     data_impute = np.hstack((data_knn_impute, data_itr_impute))
#     reg = LinearRegression().fit(data_impute, data_full[:, :3])
#     data_impute = data_impute @ reg.coef_.T

#     data_impute = np.round(data_impute)
#     data_impute = np.hstack((data_impute, data_miss[:, -2:]))

#     hybrid_impute_scores = mean_squared_error(data_full, data_impute)
#     return hybrid_impute_scores


# scores.append(get_impute_hybrid(data_miss, data_full))
# x_labels.append('hybrid')
...
for trial in range(num_trial):
    ...
    # scores[4] += get_impute_hybrid(data_miss, data_full)/num_trial
